IF EXISTS (SELECT * FROM sys.procedures WHERE name='sp_employee_search_count') 
BEGIN
   DROP PROCEDURE  dbo.sp_employee_search_count
END
GO
CREATE PROCEDURE  dbo.sp_employee_search_count
...
IF @query IS NULL
BEGIN
SET @sql ='SELECT COUNT(id) FROM Employee';
END
ELSE
BEGIN
SET @has_w=1;
